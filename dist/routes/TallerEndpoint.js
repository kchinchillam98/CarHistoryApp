...
var _TallerController = _interopRequireDefault(require("../controller/TallerController"));
...
let TallerControllerI = new _TallerController.default(); //findAll
...
router.post('/', async (request, response) => {
  ...
  try {
    ...
    response.status(500).json({
      message: 'Ocurrio un error al crear Taller',
      error
    });
  }
});
router.put('/:id', async (request, response) => {
  ...
  try {
    ...
    console.log(`el error es ${error}`);
    response.status(500).json({
      message: 'Ocurrio un error al modificar Taller',
      error
    });
  }
}); //delete
...
router.delete('/:id', async (request, response) => {
  ...
  try {
    ...
    console.log(`el error es ${error}`);
    response.status(500).json({
      message: 'Ocurrio un error al eliminar la Taller',
      error
    });
  }
});
